...
//        CloseMenu();
    ...
    public void BuyLife( GameObject button )
    {
     ...
     public void WatchVideoRewards() {
          if(AdsManagerWrapper.INSTANCE) {
            AdsManagerWrapper.INSTANCE.ShowRewards((onAdLoded) => {}, onAdFailedToLoad => {}, (iRewards) => {
                if(iRewards.rewardsItem.Amount!=0) {
                    SoundBase.Instance.GetComponent<AudioSource>().PlayOneShot( SoundBase.Instance.coins );
                    GameObject.Find( "Canvas" ).transform.Find( "GemsShop" ).gameObject.SetActive( false );
                    InitScriptName.InitScript.Instance.AddGems(30);
                }
            });
        }
    }
