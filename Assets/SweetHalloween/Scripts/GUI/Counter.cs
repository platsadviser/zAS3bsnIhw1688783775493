  ...
  //  UILabel label;
    ...
    string GetTarget()
    {
        if( Application.loadedLevelName == "map" )
        {
            ...
            else if( InitScript.Instance.currentTarget == Target.Cherry ) return "Rescue the Big Cherry Monster";

        }
        else
        {
            if( LevelData.mode == ModeGame.Vertical ) return "Clear the top";
            else if( LevelData.mode == ModeGame.Rounded ) return "Rescue the Big Cherry Monster";
