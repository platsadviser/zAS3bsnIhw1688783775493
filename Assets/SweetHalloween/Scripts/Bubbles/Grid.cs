...
public class Grid : MonoBehaviour
{
            ...
            if( value != null )
            {
                if( value.GetComponent<ball>() != null )
                {
                    if( !value.GetComponent<ball>().NotSorting  )
                    {
                        ...
                        if( value.tag == "centerball" ) value.GetComponent<SpriteRenderer>().sortingOrder = 100;
                    }
                }
    ...
    void Update()
    {
        if( busy == null )
        {
            ...
            {
                if( ( GamePlay.Instance.GameStatus == GameState.Playing || GamePlay.Instance.GameStatus == GameState.Win || GamePlay.Instance.GameStatus == GameState.WaitForTarget2 ) && LevelData.LimitAmount > 0 )
                {
