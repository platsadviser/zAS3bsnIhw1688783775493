...
//		arcadeMode = InitScript.Arcade;
	...
	void Update () {
        ...
        if( checkBall != null &&( GamePlay.Instance.GameStatus == GameState.Playing || GamePlay.Instance.GameStatus == GameState.WaitForTarget2 ))
        {
...
//			}
	...
	//		createBall();
        ...
        else if( LevelData.mode == ModeGame.Rounded && TargetCounter >= 1 && GamePlay.Instance.GameStatus == GameState.WaitForTarget2 )
            GamePlay.Instance.GameStatus = GameState.Win;
 ...
 //       yield return new WaitForSeconds( 0.6f );
    ...
    public void GetColorsInGame()
    {
        ...
        {
            if( item.tag == "centerball" || item.tag == "empty" || item.tag == "Ball" ) continue;
...
//        print( colorsDict.Count );
    ...
    IEnumerator CheckFreeChickenCor()
    {
        ...
        yield return new WaitForEndOfFrame();
        GamePlay.Instance.GameStatus = GameState.WaitForTarget2;
...
//            print( "check free centerball " +  balls.transform.childCount);
            foreach( Transform item in balls.transform )
            {
                if( item.tag != "Ball" && item.tag != "centerball" )
                {
                    finishGame = false;
                }
            }
        ...
        else if( finishGame )
        {
            ...
            GameObject centerball = GameObject.FindGameObjectWithTag( "centerball" );
            centerball.GetComponent<SpriteRenderer>().sortingLayerID = 2;
            Vector3 targetPos = new Vector3( 2.3f, 6, 0 );
            mainscript.Instance.TargetCounter++;
            AnimationCurve curveX = new AnimationCurve( new Keyframe( 0, centerball.transform.position.x ), new Keyframe( 0.5f, targetPos.x ) );
            AnimationCurve curveY = new AnimationCurve( new Keyframe( 0, centerball.transform.position.y ), new Keyframe( 0.5f, targetPos.y ) );
            curveY.AddKey( 0.2f, centerball.transform.position.y - 1 );
            float startTime = Time.time;
            Vector3 startPos = centerball.transform.position;
            float speed = 0.2f;
            float distCovered = 0;
            while( distCovered < 0.6f )
            {
                distCovered = ( Time.time - startTime );
                centerball.transform.position = new Vector3( curveX.Evaluate( distCovered ), curveY.Evaluate( distCovered ), 0 );
                centerball.transform.Rotate( Vector3.back * 10 );
                yield return new WaitForEndOfFrame();
            }
            Destroy( centerball );

            //if(centerball.GetComponent<SmoothMove>() == null)
            //    centerball.AddComponent<SmoothMove>();
        }
    }
