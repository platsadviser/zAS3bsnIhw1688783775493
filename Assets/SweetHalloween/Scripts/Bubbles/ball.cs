...
public class ball : MonoBehaviour
{
    ...
    void Update()
    {
        if( Input.GetMouseButtonUp( 0 ) )
        {
            GameObject ball = gameObject;
              if( !launched && !ball.GetComponent<ball>().setTarget && mainscript.Instance.newBall2 == null /*&& mainscript.Instance.newBall == null*/ && newBall && !Camera.main.GetComponent<mainscript>().gameOver && ( GamePlay.Instance.GameStatus == GameState.Playing || GamePlay.Instance.GameStatus == GameState.WaitForTarget2 ) )
            {
                charAnim.ShootBubble();
            // ShootBall();
            StartCoroutine(charAnim.WaitForAnimation(() => {
                Debug.Log("shoot ball sini");
               ShootBall();
            }));
            }
    ...
    public void SetBoost( BoostType boostType )
    {
        ...
        {
            GameObject Fireball = Instantiate( Resources.Load( "Particles/Fireball" ) ) as GameObject;
            Fireball.transform.parent = gameObject.transform;
            Fireball.transform.localPosition = Vector3.zero + Vector3.back * 10;
    ...
    {
        if(( mainscript.Instance.TopBorder.transform.position.y - transform.position.y <= 0.5f && LevelData.mode != ModeGame.Rounded ) || ( LevelData.mode == ModeGame.Rounded && tag == "centerball" ) )
        {
    ...
    void OnTriggerEnter2D( Collider2D other )
    {
        ...
        if( other.gameObject.name.Contains( "ball" ) && setTarget && name.IndexOf( "bug" ) < 0 )
        {
            if( !other.gameObject.GetComponent<ball>().enabled )
            {
                ...
                {
                    if( other.gameObject.tag.Contains( "animal" ) || other.gameObject.tag.Contains( "empty" ) || other.gameObject.tag.Contains( "centerball" ) ) return;
                    fireBallLimit--;
                    if( fireBallLimit > 0)
    ...
    void DestroyAround()
    {
        ...
        {
            GameObject obj = obj1.gameObject;
            if( !findInArray( b, obj ) && obj.tag != "centerball" && !obj.tag.Contains( "animal" ) && !obj.tag.Contains( "empty" ) )
            {
                b.Add( obj );
            }
        }
